...
export const ShareDialog: Component<ShareDialogProps> = (props) => {
	...
	const selectAllWorkspaces = () => {
		setSelectedWorkspaces(
			new Set<number>(filteredWorkspaces().map((w) => w.id)),
		);
	};
	...
	const selectAllThemes = () => {
		setSelectedThemes(
			new Set<number>(filteredThemes().map((theme) => theme.id)),
		);
	};
	...
	const handleExport = async (toClipboard = false) => {
		try {
			...
			if (selectedWorkspaces().size > 0) {
				exportData.workspaces = workspaceStore.store.workspaces.filter((w) =>
			...
			if (selectedTools().size > 0) {
				exportData.tools = toolStore.tools.filter((t) =>
		...
		<Dialog open={open()} onOpenChange={setOpen}>
			...
			<DialogContent class="max-w-3xl max-h-[80vh] flex flex-col">
				<DialogHeader>
					...
					<DialogDescription>
						Select workspaces and tools to export. You can save to a file or
						copy to clipboard.
					</DialogDescription>
				</DialogHeader>
				...
				<div class="flex justify-between pt-4 border-t">
					...
					<div class="flex gap-2">
						...
						<Button onClick={() => handleExport(false)}>
							<span class="iconify w-4 h-4 mr-2" data-icon="mdi:download" />{" "}
							Export to File
						</Button>
					</div>
				</div>
