...
export const ShareDialog: Component<ShareDialogProps> = (props) => {
	...
	const [selectedThemes, setSelectedThemes] = createSignal<Set<number>>(new Set<number>());
	...
	const filteredThemes = createMemo(() => {
		...
		return themeStore.themes.filter(
			(theme) =>
				theme.name.toLowerCase().includes(query) || (theme.description?.toLowerCase().includes(query) ?? false),
		);
	});
	...
	const handleExport = async (toClipboard = false) => {
		try {
			...
			if (selectedThemes().size > 0) {
				exportData.themes = themeStore.themes.filter((theme) => selectedThemes().has(theme.id));
			}

			if (!exportData.workspaces?.length && !exportData.tools?.length && !exportData.themes?.length) {
				showToast({
			...
			if (toClipboard) {
				...
				showToast({
					...
					description: `Copied ${
						(exportData.workspaces?.length || 0) + (exportData.tools?.length || 0) + (exportData.themes?.length || 0)
					} items`,
					variant: "success",
				});
			} else {
			...
			<DialogContent class="max-w-3xl max-h-[80vh] flex flex-col">
				...
				<div class="flex-1 overflow-y-auto space-y-4">
					...
					<div class="space-y-2">
						<div class="flex items-center justify-between">
							...
							<div class="flex gap-2">
								<Button variant="ghost" size="sm" onClick={selectAllThemes} disabled={filteredThemes().length === 0}>
									Select All
								</Button>
								<Button variant="ghost" size="sm" onClick={deselectAllThemes} disabled={selectedThemes().size === 0}>
									Deselect All
								</Button>
							</div>
						</div>
						<div class="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-3">
							<Show
								...
								fallback={<p class="text-sm text-muted-foreground text-center py-4">No themes found</p>}
							>
				...
				<div class="flex justify-between pt-4 border-t">
					<div class="text-sm text-muted-foreground">
						Selected: {selectedWorkspaces().size} workspaces, {selectedTools().size} tools, {selectedThemes().size}{" "}
						themes
					</div>
					<div class="flex gap-2">
